...
namespace Dolphin.Classes
{
    public class ProcessMemory
    {
        ...
        public object ReadBool(int pOffset) =>
            BitConverter.ToBoolean(ReadMem(pOffset, 4), 0);
        ...
        public void WriteBool(int pOffset, bool val)
        {
            byte[] bytes = BitConverter.GetBytes(val);
            Write(pOffset, bytes);
        }

        public byte[] Read(int adr, int l)
        {
            byte[] lpBuffer = new byte[(l - 1) + 1];
            ReadProcessMemory(this.processHandle, adr, lpBuffer, lpBuffer.Length, 0);
            return lpBuffer;
        }
        public object ReadGlowInt(int adr, int l) =>
            BitConverter.ToInt32(Read(adr, l), 0);
        public void Write(int pOffset, byte[] Bytes)
        {
            WriteProcessMemory(this.processHandle, pOffset, Bytes, Bytes.Length, 0);
        }

        public void WriteSingle(int adr, int l, float val)
        {
